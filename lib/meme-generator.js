...
const memeGenerator = (filePath, outputPath, textObj) => {
  return new Promise((resolve,reject) => {
    ...
    imagemagickCli.exec(`convert ${escaper}( "${filePath}" -coalesce -resize ${resizedImageSize.width}x${resizedImageSize.height} -layers optimize ${escaper}) -stroke black -fill white -font "./impact.ttf" ${escaper}( -strokewidth ${fontSize.top / 40} -pointsize ${fontSize.top} -annotate +0+10 "${textObj.top}" -gravity south ${escaper}) ${escaper}( -strokewidth ${fontSize.bottom / 40} -pointsize ${fontSize.bottom} -annotate +0+${10 - (fontSize.bottom / 20)} "${textObj.bottom}" -gravity north ${escaper}) -limit memory 62MiB "${outputPath}"`)
    .then((stdout, sterr) => {
