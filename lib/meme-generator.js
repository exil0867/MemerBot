...
const memeGenerator = (buffer, dirPath, ext, memeText) => {
  return new Promise((resolve,reject) => {
    ...
    function generate(inputPath) {
      ...
      imagemagickCli.exec(`convert ${escaper}( "${inputPath}" -coalesce -resize ${newWidth}x${newHeight} ${escaper}) -stroke black -fill white -font "./impact.ttf" ${escaper}( -strokewidth ${fontSize.top / 20} -pointsize ${fontSize.top} -annotate +0+10 "${memeText.top}" -gravity south ${escaper}) ${escaper}( -strokewidth ${fontSize.bottom / 25} -pointsize ${fontSize.bottom} -annotate +0+${10 - (fontSize.bottom / 20)} "${memeText.bottom}" -gravity north ${escaper}) "${unoptimizedOutputPath}"`)
      .then(() => {
