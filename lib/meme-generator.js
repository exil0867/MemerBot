...
const memeGenerator = (filePath, outputPath, textObj) => {
  return new Promise((resolve,reject) => {
    ...
    const cmd = `convert ( https://media1.tenor.com/images/0699328ffb91c331840d3141ebe08619/tenor.gif -coalesce -resize ${resizedImageSize.width}x${resizedImageSize.height} ) -stroke #000000 -fill white -font ./impact.ttf ( -strokewidth ${fontSize.top / 40} -pointsize ${fontSize.top} -annotate +0+10 "${textObj.top}" -gravity south ) ( -strokewidth ${fontSize.bottom / 40} -pointsize ${fontSize.bottom} -annotate +0+${10 - (fontSize.bottom / 20)} "${textObj.bottom}" -gravity north ) bruh.gif`;
    console.log(cmd);
    imagemagickCli.exec(cmd)
    .then((stdout, sterr) => {
