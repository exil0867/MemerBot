...
const memeGenerator = (filePath, outputPath, textObj) => {
  return new Promise((resolve,reject) => {
    ...
    const fontSize = setFontSize(textObj, { width: newWidth, height: newHeight });
    const platform = process.platform;
    const escaper = (platform === 'win32') ? '' : '\\';
    imagemagickCli.exec(`convert ${escaper}( "${filePath}" -coalesce -resize ${newWidth}x${newHeight} ${escaper}) -stroke black -fill white -font "./impact.ttf" ${escaper}( -strokewidth ${fontSize.top / 40} -pointsize ${fontSize.top} -annotate +0+10 "${textObj.top}" -gravity south ${escaper}) ${escaper}( -strokewidth ${fontSize.bottom / 40} -pointsize ${fontSize.bottom} -annotate +0+${10 - (fontSize.bottom / 20)} "${textObj.bottom}" -gravity north ${escaper}) "${outputPath}"`)
    .then((stdout, sterr) => {
