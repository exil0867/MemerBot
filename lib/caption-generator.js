...
const captionGenerator = (filePath, outputPath, captionPath, captionText) => {
  return new Promise((resolve,reject) => {
    ...
    imagemagickCli.exec(`convert -background white  -fill black  -font "./FuturaLTCondensedBold.ttf" -stroke black -strokeWidth 0 -pointsize 33 -size ${resizedImageSize.width}x -gravity Center  caption:"${captionText}" PNG24:${captionPath}`)
    .then((stdout, sterr) => {
      const captionSize = sizeOf(captionPath);
      imagemagickCli.exec(`convert ${captionPath} -gravity Center -extent ${captionSize.width}x${captionSize.height + 50} PNG24:${captionPath}`)
      .then((stdout, sterr) => {
