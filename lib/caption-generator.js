...
const captionGenerator = (filePath, outputPath, captionPath, captionText) => {
  return new Promise((resolve,reject) => {
    ...
    imagemagickCli.exec(`convert -background white  -fill black  -font "./AcuminProCond-Bold.ttf" -pointsize 37 -size ${newWidth - 50}x -gravity Center  caption:"${captionText}" PNG24:${captionPath}`)
    .then((stdout, sterr) => {
      const captionSize = sizeOf(captionPath);
      imagemagickCli.exec(`convert ${captionPath} -gravity Center -extent ${captionSize.width + 50}x${captionSize.height + 75} PNG24:${captionPath}`)
      .then((stdout, sterr) => {
