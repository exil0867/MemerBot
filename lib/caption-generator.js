...
const captionGenerator = (buffer, dirPath, ext, captionText) => {
  return new Promise(async (resolve, reject) => {
    ...
    function generate(inputPath) {
      ...
      imagemagickCli.exec(`convert -background white -interline-spacing 0 -kerning 0 -fill black  -font "./caption-font.otf" -pointsize 34 -size ${newWidth - 50}x -gravity Center  caption:"${captionText}" PNG24:${captionPath}`)
        .then(() => {
          const captionSize = sizeOf(captionPath);
          return imagemagickCli.exec(`convert ${captionPath} -gravity Center -extent ${captionSize.width + 50}x${captionSize.height + 60} ${escaper} PNG24:${captionPath}`)
        }).then(() => {
          const unoptimizedOutputPath = path.resolve(dirPath, `unoptimized-output.${ext}`)
          return imagemagickCli.exec(`convert ${captionPath} -alpha Copy -background Transparent ${escaper}( ${inputPath} -coalesce -resize ${newWidth}x${newHeight} ${escaper}) -set page "%[fx:u.w]x%[fx:u.h+v.h]+%[fx:t?(u.w-v.w)/2:0]+%[fx:t?u.h:0]" -coalesce null: -insert 1 -layers composite -loop 0 -define colorspace:auto-grayscale=off ${unoptimizedOutputPath}`).then(() => (unoptimizedOutputPath));
        }).then((unoptimizedOutputPath) => {
