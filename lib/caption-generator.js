...
const captionGenerator = (buffer, dirPath, ext, captionText) => {
  return new Promise(async (resolve, reject) => {
    ...
    function generate(inputPath) {
      ...
      imagemagickCli.exec(`convert -background white  -fill black  -font "./AcuminProCond-Bold.ttf" -pointsize 37 -size ${imageSize.width + 35}x -gravity Center  caption:"${captionText}" PNG24:${captionPath}`)
        .then(() => {
          const captionSize = sizeOf(captionPath);
          return imagemagickCli.exec(`convert ${captionPath} -gravity Center -extent ${captionSize.width + 35}x${captionSize.height + 50} PNG24:${captionPath}`)
        }).then(() => {
