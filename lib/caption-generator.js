...
const captionGenerator = (buffer, dirPath, ext, captionText) => {
  return new Promise(async (resolve, reject) => {
    ...
    function generate(inputPath) {
      ...
      imagemagickCli.exec(`convert -background white -interline-spacing 0 -kerning 0 -fill black  -font "./caption-font.otf" -pointsize 41 -size ${imageSize.width}x -gravity Center  caption:"${captionText}" PNG24:${captionPath}`)
        .then(() => {
          const captionSize = sizeOf(captionPath);
          return imagemagickCli.exec(`convert ${captionPath} -gravity Center -extent ${captionSize.width + 30}x${captionSize.height + 75} ${escaper}( -resize ${captionSize.width}x${captionSize.width} ${escaper}) PNG24:${captionPath}`)
        }).then(() => {
