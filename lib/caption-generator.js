...
const captionGenerator = (buffer, dirPath, ext, captionText) => {
  return new Promise(async (resolve, reject) => {
    ...
    .catch((err) => {
      reject(err);convert -background white -interline-spacing 8 -kerning 1  -fill black  -font "./AcuminProCond-Bold.ttf" -pointsize 34 -size 300x -gravity Center  caption:"melon funny" PNG24:output.gif
    ...
    function generate(inputPath) {
      ...
      imagemagickCli.exec(`convert -background white -interline-spacing 8 -kerning 0.25 -fill black  -font "./AcuminProCond-Bold.ttf" -pointsize 34 -size ${imageSize.width}x -gravity Center  caption:"${captionText}" PNG24:${captionPath}`)
        .then(() => {
          const captionSize = sizeOf(captionPath);
          return imagemagickCli.exec(`convert ${captionPath} -gravity Center -extent ${captionSize.width + 30}x${captionSize.height + 70} ${escaper}( -resize ${captionSize.width}x${captionSize.width} ${escaper}) PNG24:${captionPath}`)
        }).then(() => {
          const unoptimizedOutputPath = path.resolve(dirPath, `unoptimized-output.${ext}`)
          return imagemagickCli.exec(`convert ${captionPath} -alpha Copy -background Transparent ${escaper}( ${inputPath} -coalesce ${escaper}) -set page "%[fx:u.w]x%[fx:u.h+v.h]+%[fx:t?(u.w-v.w)/2:0]+%[fx:t?u.h:0]" -coalesce null: -insert 1 -layers composite -loop 0 -define colorspace:auto-grayscale=off ${unoptimizedOutputPath}`).then(() => (unoptimizedOutputPath));
        }).then((unoptimizedOutputPath) => {
